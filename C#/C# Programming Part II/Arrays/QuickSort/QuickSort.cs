...
class QuickSort
{
    static void Main()
    {
        ...
        QuickSortAlgo(words, 0, words.Length - 1);

        Console.WriteLine("After sorting: {0}", string.Join(", ", words));
    }

    static void QuickSortAlgo(string[] elements, int left, int right)
    {
        ...
        if (left < rightPointer) QuickSortAlgo(elements, left, rightPointer);
        if (leftPointer < right) QuickSortAlgo(elements, leftPointer, right);
    }
}
